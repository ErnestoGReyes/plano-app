...
// PALETAS DE TEMA
// ═══════════════════════════════════════════════════════════════════════════════
// Cada tema define un par { dark, light } con exactamente las mismas claves, para
// que cualquier componente que consuma useTheme() (que sigue devolviendo un solo
// objeto de colores "C") funcione sin cambios sin importar qué tema/modo esté
// activo. Ver design/THEMES más abajo para el registro completo y ../contexts/
// ThemeContext.jsx para cómo se resuelve `themeId` + `isDark` → paleta.

// ── 1. NOIR OSCURO — negro total, dorado (tema original de Plano) ───────────
export const DARK = {
  bgApp:        "#0A0909",
  bgSidebar:    "#070706",
  bgEditor:     "#0D0B0A",
...
// ── 2. MÁQUINA DE ESCRIBIR — papel envejecido, tinta, lápiz corrector rojo ──
...
// ── 3. NEO-NOIR — negro profundo, magenta/cian eléctrico ────────────────────
...
// ── 4. BLUEPRINT TÉCNICO — azul petróleo, líneas de plano ───────────────────
...
// ── 5. TEATRO / TERCIOPELO — rojo vino, dorado, negro ───────────────────────
...
// ── 6. DESIERTO / WESTERN — terracota, ocre, arena ──────────────────────────
...
// ── 7. EDITORIAL NÓRDICO — gris pizarra / blanco, un acento verde ───────────
...
// ── Registro de temas ────────────────────────────────────────────────────────
// `swatch` es el color que se usa para pintar el círculo de vista previa en el
// selector de temas (components/ThemeSelector.jsx) — siempre el acento del
// modo oscuro de cada tema, que es el más representativo de su identidad.
export const THEMES = {
  noir:      { id:"noir",      label:"Noir Clásico",       swatch:DARK.accent,       dark:DARK,           light:LIGHT },
  vintage:   { id:"vintage",   label:"Máquina de Escribir", swatch:VINTAGE_DARK.accent,   dark:VINTAGE_DARK,   light:VINTAGE_LIGHT },
...
// Resuelve la paleta de colores activa a partir del id de tema + modo día/noche.
// Si el themeId guardado ya no existe (ej. se eliminó un tema), cae al default
...
// activo a través de useTheme(), definido en ../contexts/ThemeContext.jsx, que
// envuelve la app con un <ThemeProvider>. Esto garantiza que TODO componente
// que lo consuma se vuelva a renderizar en el instante en que cambia el tema
// (o la paleta elegida) — sin depender de mutar un objeto compartido y esperar
// que algo más dispare un re-render.
