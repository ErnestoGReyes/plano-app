        ...
        {types.map(t => {
          ...
          return (
            <button key={t.type} onClick={()=>onTypeChange(t.type)}
              title={`${t.label} (Tab)`}
              style={{padding:"4px 10px", borderRadius:RADIUS.sm, fontSize:11, fontWeight:on?700:400,
...
function ScriptBlockImpl({ block, index, isActive, characterColors, onUpdate, onFocus,
  ...
  return (
    <div style={{position:"relative", ...(wrappers[block.type]||{}),
      ...
      paddingLeft:8, paddingRight:0, borderRadius:RADIUS.xs, transition:"background .1s"}}
      ...
      {isActive && !isMobile && (
        <div style={{position:"absolute", left:-48, top:0, fontSize:8, fontWeight:700,
          ...
          background:`rgba(${hexToRgb(col)},.1)`, padding:"2px 5px", borderRadius:RADIUS.xs,
        ...
        <div style={{display:"flex", gap:4, marginTop:2, justifyContent:"flex-end"}}>
          <button onClick={e=>{e.stopPropagation();onAddBlockAfter(index);}}
            style={{background:"none",border:`1px solid ${C.border}`,
              borderRadius:RADIUS.xs,padding:"2px 8px",fontSize:10,color:C.textMuted,
          ...
          {block.text==="" && (
            <button onClick={e=>{e.stopPropagation();onDeleteBlock(index);}}
              style={{background:"none",border:`1px solid rgba(240,96,96,.25)`,
                borderRadius:RADIUS.xs,padding:"3px 7px",color:C.red,
